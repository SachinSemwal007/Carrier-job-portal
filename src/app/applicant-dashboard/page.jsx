        ...
        <div className="flex flex-col min-h-screen bg-gray-100 transition-all duration-300 ease-in-out"> 
          ...
          <header className="w-full bg-teal-50 text-white py-2 px-6 flex justify-between items-center shadow-lg"> 
            <div className="flex items-center"> 
              <Link href="/jobs"> 
                <Image src="/JSSPS.webp" alt="Company Logo" width={190} height={100} quality={100} className="h-20 w-auto object-contain" /> 
              </Link> 
              <div> 
                <h1 className="text-base ml-5 sm:text-xl font-bold text-teal-950">Jharkhand State Sports Promotion Society</h1> 
                <p className="text-sm ml-5 sm:text-md text-teal-950">(A State Govt. of Jharkhand and CCL Joint Initiative)</p> 
              </div> 
            </div> 
                  ...
                  <ul> 
                    ...
                    <li> 
                      <Link 
                        href="/" 
                        className={`block p-2 rounded transition duration-300    
                    ${active === "logout" ? "bg-red-600 text-white" : "text-white hover:bg-red-500"}`} 
                      > 
            ...
            <main className="flex-grow p-4 lg:p-8 transition-all duration-300 ease-in-out"> 
              {active === "" ? ( 
                <div className="text-center"> 
                  <Image src="/JSSPS.webp" alt="Logo" width={220} height={120} className="mb-4 h-auto w-auto mx-auto" /> 
                ...
                <div className="bg-white p-6 rounded-lg shadow-md w-full"> 
                        ...
                        return ( 
                          <div key={index} className="m-5"> 
                            <div className="p-4 bg-white rounded-lg shadow-lg mb-4"> 
                              {/* Submission Status */} 
                              <h2 className={`text-lg font-semibold ${item.submitted ? "text-green-600" : "text-yellow-500"}`}>{item.submitted ? "Application Submitted Successfully" : "Saved Draft"}</h2> 
 
                              {/* Application ID */} 
                              <h2 className="text-gray-700 text-sm mt-2"> 
                                <span className="font-semibold">Application ID:</span> {item.applicationId} 
                              </h2> 
 
                              {/* Application Title */} 
                              <h2 className="text-gray-700 text-sm mb-4"> 
                                <span className="font-semibold">Title:</span> {checkTitle(item.applicationId)} 
                              </h2> 
                  ...
                  {active === "change-password" && ( 
                    <div className="bg-white p-4 rounded-md shadow-md max-w-md mx-auto"> 
                      ...
                      <p className="text-gray-700 mb-2">Are you sure you want to change your password?</p> 
                      <div className="flex gap-4"> 
                        <button onClick={handleForgotPassword} className="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition duration-300"> 
                          Yes 
                        </button> 
                        <button onClick={handleCancel} className="bg-gray-300 text-gray-700 py-2 px-4 rounded hover:bg-gray-400 transition duration-300"> 
                          No 
                        </button> 
                      </div> 
                      {message && <span className="block mt-4 text-green-500">{message}</span>} 
                    </div> 
                  )} 
