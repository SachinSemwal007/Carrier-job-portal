...
const AdminDashboard = () => {
  ...
  const fetchAdmins = async () => {
    try {
      const response = await fetch(
        "https://9dwb3ngewc.execute-api.ap-south-1.amazonaws.com/dev/api/admins"
  ...
  const handleRemoveAdmin = async (adminId) => {
    try {
      await fetch(
        `https://9dwb3ngewc.execute-api.ap-south-1.amazonaws.com/dev/api/admins/${adminId}`,
        {
          method: "DELETE",
        }
      );
              ...
              <div className="p-4">
                ...
                <nav>
                  <ul>
                    ...
                    <li>
                      <button
                        className="block p-2 rounded transition duration-300 text-gray-300 hover:bg-red-500 w-full text-left"
                        onClick={() => logout()}
                      >
                        Logout
                      </button>
            ...
            {menuOpen && (
              <div className="absolute top-16 left-0 right-0 bg-gray-800 text-white z-50 p-4 shadow-lg">
                <div className="flex items-center justify-center">
                  <p className="mr-2 text-white font-bold">{user.email}</p>
